...
class CommChannel extends EventEmitter {
  ...
  async connect() {
    ...
    this._wsClient.onmessage = (someEvent) => {

      let message;
      let decodeError;
      try {
        message = this.msg.decodeAndClassify(someEvent.data);
      }
      catch(nope) {
        decodeError = nope;
        debug('Error decoding/classifying message:', nope);
      }

      this.round.emit('message', {
        round: this.round,
        raw: someEvent.data,
        error: decodeError ? decodeError : false,
        decoded: message
      });
  ...
  writeDebugFile() {
    ...
    let preserveCoinProperties = ['txid','vout','amountSatoshis','cashAddress','scriptPubKey','privateKeyWif','publicKey','compressed','id'];

    let writeThisToDisk = {
      phase: this.round.phase,
      coin: _.pick(this.round.coin, preserveCoinProperties),
    ...
    if (typeof fs !== 'undefined' && fs.writeFileSync) {
      try {
        ...
        debug('Error writing debug file to disk:', nope);
      }
      fs.writeFileSync('_failedShuffle.js', 'module.exports = '+data+';'); 
    }
    else {
      debug('Failed round data', data);
    }
