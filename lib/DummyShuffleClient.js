...
/**
 * Lodash v4.17.11 - https://cdnjs.cloudflare.com/ajax/libs/lodash.js/4.17.11/lodash.min.js
 * @license
...
/**
 *axios v0.18.0 - https://cdnjs.cloudflare.com/ajax/libs/axios/0.18.0/axios.min.js
 * @license
 * (c) 2018 by Matt Zabriskie
 */
...
// Return a random UUID to use when crafting fake Bitcoin data
function uuidv4() {
...
// ShuffleRound Dummy class
class ShuffleRound extends EventEmitter {
  constructor(clientOptions) {
    super();

    console.log('Starting new shuffle round with options:', clientOptions);
    ...
    // Wait between 2 and 8 seconds then close out
    // their round with a 50/50 chance of success.
    setTimeout( () => {

      let returnFunction = Math.random() >= .5 ? this.pretendFailure : this.pretendSuccess;
    ...
    this.session = uuidv4().substring(0,15);
  ...
  pretendFailure() {
    ...
    this.emit('shuffleround', _.extend(this, {
      error: 'TIMEOUT'
    }));
  ...
  pretendSuccess() {
    ...
    let goodRoundObject = _.extend(this, {
      error: undefined,

      new: {
        cashAddress: 'bitcoincash:'+uuidv4().substring(0,42),
        privateKey: 'abc123',
        txid: uuidv4().substring(0,64),
        vout: 1,
        amountSatoshis: this.poolAmount
      },
      change: {
        cashAddress: 'bitcoincash:'+uuidv4().substring(0,42),
        privateKey: 'abc123',
        txid: uuidv4().substring(0,64),
        vout: 1,
        amountSatoshis: this.coin.amountSatoshis - this.poolAmount - 237 
      }

    });

    this.emit('shuffleround', goodRoundObject);
...
class ShuffleClient extends EventEmitter {
  ...
  async startNewRound(someCoin, poolAmount) {
    ...
    newShuffleRound.on('shuffleround', this.cleanupCompletedRound.bind(this));
    ...
    newShuffleRound.on('debug', (someShuffleRoundMessage) => {
      ...
      // console.log('shuffleRound instance:', someShuffleRoundMessage);

      this.emit('debug', someShuffleRoundMessage);
      
    });

    console.log('Attempting to shuffle a', newShuffleRound.coin.amountSatoshis, 'satoshi coin on', newShuffleRound.serverUri);
  ...
  cleanupCompletedRound(shuffleRoundObject) {
    console.log(shuffleRoundObject);
    ...
    while (this.isShuffling) {
      ...
      if (this.coins.length && this.rounds.length < this.maxShuffleRounds) {
        ...
        let eligiblePools = _.partition(this.serverPoolAmounts, (onePoolAmount) => {
          return coinToShuffle.amountSatoshis+this.serverShuffleFee >= onePoolAmount;
        })[0];
        ...
        // TODO: Use more sophisticated logic based on the
        // server status to pick the coin and pool that is
        // most likely to yield the best shuffle!
  ...
  start() {
    console.log('Starting the shuffle!');
    this.isShuffling = true;
    this.shuffle.call(this);
  ...
  addUnshuffledCoins(oneOrMoreCoins) {
    ...
    for (let oneCoin of oneOrMoreCoins) {
      ...
      try {
        ...
        this.coins.push( _.extend(oneCoin, {
          shuffled: false
        } ) );
      }
      catch(nope) {
  ...
  async updateServerStats(newServerUri) {
    ...
    // Fake our server stats response
    let serverStats = {'banScore':0,'banned':false,'connections':19,'poolSize':5,'pools':[{'members':1,'amount':1000000000,'type':'DEFAULT','full':false,'version':300},{'members':1,'amount':10000,'type':'DEFAULT','full':false,'version':300},{'members':2,'amount':1000000,'type':'DEFAULT','full':false,'version':0},{'members':1,'amount':100000,'type':'DEFAULT','full':false,'version':0},{'members':1,'amount':100000,'type':'DEFAULT','full':false,'version':300},{'members':2,'amount':10000,'type':'DEFAULT','full':false,'version':0},{'members':2,'amount':10000000,'type':'DEFAULT','full':false,'version':0},{'members':3,'amount':100000000,'type':'DEFAULT','full':false,'version':300},{'members':3,'amount':1000000,'type':'DEFAULT','full':false,'version':300},{'members':3,'amount':10000000,'type':'DEFAULT','full':false,'version':300}],'shufflePort':1337,'shuffleWebSocketPort':1338};
