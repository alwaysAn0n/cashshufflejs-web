    ...
    while (this.isShuffling) {
      ...
      if (!this.serverBackoffMs) {
        ...
        if (this.coins.length && this.rounds.length < this.maxShuffleRounds) {
          ...
          const sereverUriWithDefault = this.serverUri || `ws://${this.serverStatsUri.replace(/(\w+:\/\/)([^:]+)(:.+)/i, '$2')}:${this.serverStats.shuffleWebSocketPort}`;

          // console.log('Starting new round in:', serverUri);
          try {
            await this.startNewRound(coinToShuffle, poolToUse, sereverUriWithDefault);
          }
          catch(nope) {
