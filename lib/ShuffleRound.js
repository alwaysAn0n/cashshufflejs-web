...
class ShuffleRound extends EventEmitter {
  ...
  async actOnMessage(jsonMessage) {
    ...
    switch (messageType) {
      ...
      break;
      case 'incomingOutputAndSig':
        this.verifyAndSubmit(message);
      break;
      // case '':
      // break;
      default:
...
addPlayerToRound(message) {
  ...
  */
  ...
  processEquivCheckMessage(prunedMessage) {
    ...
    if ( allHashes.length === this.players.length) {
      ...
      if (_.uniq(allHashes).length === 1 && _.uniq(allHashes)[0] === this.equivHash) {
        ...
        let mySignatureString = 'seems legit';
        ...
        this.comms.sendMessage('broadcastSignatureAndUtxo',
          ...
          mySignatureString, this.phase, this.ephemeralKeypair.publicKey,
          this.ephemeralKeypair.privateKey
        );
  ...
  verifyAndSubmit(prunedMessage) {

    console.log('\t****** WE ARE IN THE LAST STEP! *******\n');
