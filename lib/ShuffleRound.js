...
class ShuffleRound extends EventEmitter {
  constructor(clientOptions) {
    ...
    // The coin that will be shuffled in this round.
    this.coin = clientOptions.coin;

    if (!_.isObject(this.coin)) {
      debug(`A valid coin object was not provided!`);
      throw new Error('BAD_COIN');
    };

    const returnAsync = function(someFunction, callFnWithParam) {

      const returnVal = someFunction(callFnWithParam);

      const isPromise = !!returnVal && (typeof returnVal === 'object' || typeof returnVal === 'function') && typeof returnVal.then === 'function';
    ...
    returnAsync(this.hooks.shuffled, this.coin)
    .then((someObject) => {
      _.extend(this.shuffled, someObject);
    })
    .catch((someError) => {
      debug('Error getting shuffled output details:', someError);
    });
    ...
    returnAsync(this.hooks.change, this.coin)
    .then((someObject) => {
      _.extend(this.change, someObject);
    })
    .catch((someError) => {
      debug('Error getting change output details:', someError);
    });
    ...
    this.comms.on('disconnected', (commsDisconnectMessage) => {
      ...
      if (this.roundComplete) {
        ...
        // Until we have a good way to distinguish between network problems
        // and the server timing out, assume it's a timeout if there are no
        // players in the round yet.
        _.extend(this.roundError, {
          shortCode: this.players.length ? 'COMMS_DISCONNECT' : 'ROUND_TIMEOUT',
          errorObject: {},
          isProtocolError: false,
          isException: false
        });
  ...
  handleCommsError(someError) {
    debug('Something has gone wrong with our communication channel:', someError);
  ...
  setPhase(somePhaseName) {

    this.phase = somePhaseName;

    this.emit('phase', {
      message: somePhaseName,
      round: this
    });
...
async addPlayerToRound(message) {
    ...
    if (playerToAdd.isMe) {
      let ourCoin = this.coin;
      ourCoin.publicKey = this.coin.publicKey.toString('hex');
      _.extend(playerToAdd.coin, ourCoin);
    }
  ...
  */
  checkFinalOutputsAndDoEquivCheck(signedPackets) {
    ...
    this.setPhase('EQUIVOCATION_CHECK');
  ...
  async processEquivCheckMessage(prunedMessage) {
    ...
    if ( allHashes.length === this.players.length) {
      ...
      if (_.uniq(allHashes).length === 1 && _.uniq(allHashes)[0] === this.equivHash) {
        ...
        this.setPhase('VERIFICATION_AND_SUBMISSION');
  ...
  */
  ...
  abortRound() {
    debug('The user has aborted the shuffle');

    this.roundError = {
      shortCode: 'USER_ABORT',
      errorObject: {},
      isProtocolError: false,
      isException: false
    };
