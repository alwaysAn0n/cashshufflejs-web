  ...
  async actOnMessage(jsonMessage) {
    ...
    switch (messageType) {
      ...
      case 'incomingEquivCheck':
        ...
        this.processEquivCheckMessage(message);
...
addPlayerToRound(message) {
    ...
    if (playerToAdd.isMe) {
      _.extend(playerToAdd.coin, {
        privateKeyWif: new bch.PrivateKey(this.coin.privateKey).toWIF()
      })
    }
  ...
  announceChangeAddress() {
    ...
    // TODO: I think this should be negated
    if (me){
  ...
  */
  checkFinalOutputsAndDoEquivCheck(signedPackets) {
    ...
    this.equivHash = bch.crypto.Hash.sha256sha256(Buffer.from(this.equivHashPlaintext, 'utf8')).toString('base64');
  ...
  //
  processEquivCheckMessage(prunedMessage) {
    ...
    console.log('Got an processEquivCheckMessage from', sender.verificationKey, 'with hash', sender.equivCheck);
    ...
    if ( allHashes.length === this.players.length) {
      ...
      if (_.uniq(allHashes).length === 1 && _.uniq(allHashes)[0] === this.equivHash) {
        console.log('Everyone passess the EQUIVOCATION_CHECK!');
  ...
  async verifyAndSubmit(prunedMessage) {
    ...
    if (! this.comms.outbox.sent['broadcastSignatureAndUtxo']) {
      ...
      console.log('Shuffle Transaction:\n', shuffleTransaction.hex);
      console.log('\n\nShuffle Transaction Signature:', shuffleTransaction.signatureBase64);
      // console.log('Broadcasting our sig and utxo');

      // broadcastSignatureAndUtxo(session, fromPlayerNumber, coinUtxoData, signatureString, phase, myVerificationPubKey, myVerificationPrivKey) {
    ...
    // Add the hash provided by the player to that player's state data
    let sender = _.extend( this.players[_.findIndex(this.players, { session: prunedMessage['session'] } )] , {
      signatures: _.get(prunedMessage, 'message.signatures')
    });

    for (let oneSig of sender.signatures) {
      console.log('Got a signature message from', sender.verificationKey, 'for coin', oneSig.utxo, 'and has sig', oneSig.signature);
    }
