...
module.exports = {
  ...
  prepareShuffleInsAndOuts: async function(options) {
    ...
    let players = _.map(options.players, function(onePlayer) {
      ...
      playerPubKey = _.isString(playerPubKey) ? playerPubKey : typeof playerPubKey.toString === 'function' ? playerPubKey.toString() : undefined;

      if (!playerPubKey) {
        throw new Error('BAD_PUBKEY');
      }

      let pubKey = new PublicKey(pubkeyString);
