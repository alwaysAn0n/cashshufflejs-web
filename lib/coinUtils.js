...
module.exports = {
  ...
  getCoinDetails: async function(someTxid, someVout) {
    ...
    try {
      ...
      debug('Something went wrong fetching transaction data');
    ...
    try {
      ...
      debug('Something went wrong fetching utxo data:', nope.message);
      throw new Error('BAD_COIN');
    }
  ...
  prepareShuffleInsAndOuts: async function(options) {
    ...
    let players = _.map(options.players, function(onePlayer) {
      // TODO: this check should be performed much earlier.
      let playerPubKey = onePlayer.coin.publicKey;
      playerPubKey = _.isString(playerPubKey) ? playerPubKey : typeof playerPubKey.toString === 'function' ? playerPubKey.toString('hex') : undefined;

      if (!playerPubKey) {
        throw new Error('BAD_PUBKEY');
      }

      let pubKey = new PublicKey(playerPubKey);
    ...
    // debug('Fetching utxos for:', addressesToFetch);
    ...
    try {
      ...
      debug('Something went wrong fetching utxo data:', nope);
      throw nope;
    }
  ...
  buildShuffleTransaction: async function(options) {
    ...
    try {
      ...
      debug('cannot prepare inputs and outputs for shuffle Transaction');
      throw nope;
    }
