...
const typeDefinitions = require('./message.proto.js');
...
const PB = {
  ...
  root: protobuf.Root.fromJSON(typeDefinitions)
};
...
function decodeAndClassify(messageBuffer) {
  // In the browser, the ws library only gives us ArrayBuffers.
...
function forwardEncryptedOutputs(session, fromPlayerNumber, arrayOfOutputs, phase, toVerificationKey, myVerificationPubKey, myVerificationPrivKey) {
  ...
  for (let oneEncryptedAddress of arrayOfOutputs) {
    ...
    let msg = Buffer.from(PB.Packet.encode(message.packet).finish()).toString('base64');
...
function checkPacketSignature(oneSignedPacket) {
  ...
  let message = Buffer.from(packet.finish()).toString('base64');
  let result = false;
console.log('Checking packet signature');
console.log('Base64 Signature is:', signature);
...
function blameMessage(options, mySessionId, myPlayerNumber, myVerificationPublicKey, myVerificationPrivateKey) {
  ...
  let blameMessage = _.reduce(_.keys(options), function(msg, oneOptionName) {
    ...
    switch (oneOptionName) {
      ...
      break;
      case 'invalid':
        msg.packet.message.blame.invalid = PB.Invalid.create({ invalid: options.invalid });
      break;
      case 'hash':
  ...
  let msg = Buffer.from(PB.Packet.encode(blameMessage.packet).finish()).toString('base64');
